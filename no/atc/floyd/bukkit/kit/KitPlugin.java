package no.atc.floyd.bukkit.kit;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args ) {
      String cmdname = cmd.getName().toLowerCase();
        ...
        if (cmdname.equalsIgnoreCase("kit")) {
          if (player == null || player.hasPermission("kit") || player.hasPermission("kit.kit")) {
              if (args.length == 0) {
              ...
              for ( KitObject kit : kits ) {
                ...
                if (player == null || player.hasPermission("kit." + kit.Name().toLowerCase()) ) {
                  if (!kitnames.matches("")) {
...
*/                }
              ...
              if (args.length >= 1) {
                if (args[0].equalsIgnoreCase("reload") && (player == null || player.hasPermission("kit.reload"))) {
                  kits.clear();
              ...
              for ( KitObject kit : kits ) {
                ...
                logger.finest("[Kit] Found kit '"+kit.Name().toLowerCase()+"', checking access");
                  if (player == null) {
                    ...
                    if (player.hasPermission("kit." + kit.Name().toLowerCase())) {
                      ...
                      return true;
                          ...
                          if (kit.Cooldown() != 0) {
                            ...
                            if (player.hasPermission("kit.proxy") && seconds > 0) {
...
*/                          
                    ...
                    for (String playername : players) {
                      ...
                      if (p != null) {
                        if (player == null || p.equals(player) || player.hasPermission("kit.proxy")) {
    ...
    private void loadKits() {
      ...
      try {
        output = new BufferedWriter(new FileWriter(fname));
        output.write("# Name;ID Amount;ID Amount;ID amount (etc)[;-cooldown][;$price]" + newline);
